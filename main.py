...
def get_messages_document(message):
    if message.document.file_size > 500:
        dailySerbian_bot.send_message(CURRENT_USER_ID(message), "в рот я ебал такие большие файлы качать!")
        return None
...
def get_messages_voice(message):
    if message.voice.duration > 30:
        dailySerbian_bot.send_message(CURRENT_USER_ID(message), "в рот я ебал такие длинные сообщения слушать!")
        return None
...
def check_button_ask_add2dict_no(call):
    dailySerbian_bot.send_message(CURRENT_USER_ID(call), "Ну и хуй с тобой! Оствайся тупым!")
