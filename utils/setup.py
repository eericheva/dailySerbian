...
dailySerbian_updater = Updater('6585528539:AAFeOHutk007_lryWvihnI3lKn-W7O0XT0w', use_context=True)
...
my_users_files = [s for s in glob.glob(os.path.join(USER_DICT_PATH, "*")) if s.endswith(my_user_ends)]
MY_USERS = [s.split("/")[-1].split(my_user_ends)[0] for s in my_users_files]

CURRENT_USER_ID = lambda message: message.from_user.id if isinstance(message, telebot.types.Message) else int(message.split(".")[0])
CURRENT_USER_USERNAME = lambda message: message.from_user.username if isinstance(message, telebot.types.Message) else message.split(".")[1]

CURRENT_USER = lambda message: str(CURRENT_USER_ID(message)) + "." + CURRENT_USER_USERNAME(message)
CURRENT_USER_FILE = lambda message: CURRENT_USER(message) + my_user_ends
CURRENT_USER_DICT = lambda message: json.load(open(os.path.join(USER_DICT_PATH, CURRENT_USER_FILE(message)), "r"))
